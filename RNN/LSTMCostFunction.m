function [cost, grad, ht, ct] = LSTMCostFunction(W, xt, yt, h_prev, c_prev)
    ...
    [wxc, whc, forget] = unroll(C, d, n, true);
    ...
    ct = ft.*c_prev + it.*tanh(ch);
    ot = logistic(wxo*xt + who*h_prev + wco*ct + bo);
    ht = ot.*tanh(ct);
    y_temp = why*ht + by;
    exp_term = exp(y_temp);
    yh = exp_term./sum(exp_term);
    ...
    d_ct = wco'*((ot.*(1 - ot)).*tanh(ct).*d_ht) + (ot.*(1 - tanh(ct).^2)).*d_ht; % dx1
    d_wxo = (ot.*(1 - ot).*d_ot)*xt'; % dxn 
    d_who = (ot.*(1 - ot).*d_ot)*h_prev'; % d<xd 
